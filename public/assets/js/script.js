...
(function ($) {
    ...
    $(document).on('submit', '[id^=vendor-form]', function (e) {
          e.preventDefault();
          var data = $(this).serialize();
          $('.btn-submit-vendors').prop('disabled', true);
          $(".btn-submit-vendors").empty();
          $('.btn-submit-vendors').append("<span class='spinner-border spinner-border-sm' role='status' aria-hidden='true'></span> &nbsp; Loading...");
             ...
             $.ajax({
                    ...
                    success: function (response) {
                       $('.btn-submit-vendors').prop('disabled', false);
                       $(".btn-submit-vendors").empty();
                       $('.btn-submit-vendors').append("Register as Vendor");
                        if (response === 'ok') {
                            $.notify({
                                icon: 'fa fa-check',
                                title: 'Vendor Registration Success ! Please check your email.',
                                message: ''
                            }, {
                                element: 'body',
                                position: null,
    ...
    $(document).on('submit', '[id^=addbookingform]', function (e) {
          e.preventDefault();
          var data = $(this).serialize();
            $('.btn-add-booking').prop('disabled', true);
            $('.btn-add-booking').empty();
            $('.btn-add-booking').append("<span class='spinner-border spinner-border-sm' role='status' aria-hidden='true'></span> &nbsp; Loading...");
             $.ajax({
                    type: 'POST',
                    url: "add-booking",
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
    ...
    $(document).on('submit', '[id^=addtocartForm]', function (e) {
          e.preventDefault();
          var data = $(this).serialize();
          $('.btn-add-to-cart').prop('disabled', true);
          $('.btn-add-to-cart').empty();
          $('.btn-add-to-cart').append("<span class='spinner-border spinner-border-sm' role='status' aria-hidden='true'></span> &nbsp; Loading...");
             $.ajax({
                    type: 'POST',
                    url: "../add-cart",
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    ...
                    success: function (response) {
                        ...
                        $(".btn-add-to-cart").empty();
                        $('.btn-add-to-cart').append("Add To Cart");
                        ...
                        if (response === 'ok') {
                            $.notify({
                                ...
                                icon_type: 'class',
                                template: '<div data-notify="container" class="col-xs-11 col-sm-3 alert alert-{0}" role="alert">' +
                                    '<button type="button" aria-hidden="true" class="btn-close btn-refresh" data-notify="dismiss"></button>' +
                                    '<span data-notify="icon"></span> ' +
                                    '<span data-notify="title">{1}</span> ' +
                                    '<span data-notify="message">{2}</span>' +
                                    '<div class="progress" data-notify="progressbar">' +
    ...
    $(document).on('submit', '[id^=addtoreviewForm]', function (e) {
          e.preventDefault();
          var data = $(this).serialize();
          $('.btn-submit-review').prop('disabled', true);
          $('.btn-submit-review').empty();
          $('.btn-submit-review').append("<span class='spinner-border spinner-border-sm' role='status' aria-hidden='true'></span> &nbsp; Loading...");
             $.ajax({
                    type: 'POST',
                    url: "../add-review",
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    ...
                    success: function (response) {
                        ...
                        $(".btn-submit-review").empty();
                        $('.btn-submit-review').append("Submit Your Review");
                        $('#addtoreviewForm').trigger("reset");

                        if (response === 'ok') {
                            $.notify({
                                icon: 'fa fa-check',
                                title: 'Success submit review',
                                message: ''
                            }, {
                                element: 'body',
                                position: null,
    ...
    // $('.btn-submit-vendors').on('click', function () {
    //     $.notify({
    //         icon: 'fa fa-check',
    //         title: 'Success!',
    //         message: 'Success'
